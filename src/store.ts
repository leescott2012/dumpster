...
import { savePhotoBlob, getPhotoBlob, deletePhotoBlob, clearAllPhotos } from './db';
...
export const useStore = create<Store>((set, get) => {
  ...
  return {
    ...
    cropPhoto: (photoId, croppedBlob) => {
      ...
      // Update in IndexedDB
      savePhotoBlob(photoId, croppedBlob);
      set((s) => ({
        photos: s.photos.map((p) =>
    ...
    newDump: () => {
      ...
      const id = `d-${Date.now()}`;
      set((s) => ({
        dumps: [...s.dumps, { id, num, title: `Dump ${num.toString().padStart(2, '0')}`, photos: [], vibeBadge: null }],
        activeDumpId: id,
      }));
      persist();
    },
    ...
    autoGenerateDump: (count) => {
      snap();
      const s = get();
      const available = s.photos.filter(p => !s.dumps.some(d => d.photos.includes(p.id)));
      if (available.length < 3) return;
      const selection = available.sort(() => Math.random() - 0.5).slice(0, count);
      const arranged = arrangePhotos(selection);
      const id = `d-${Date.now()}`;
      const num = ++dumpCounter;
      const title = generateDumpTitle(arranged);
      set((s) => ({
        dumps: [...s.dumps, { id, num, title, photos: arranged.map(p => p.id), vibeBadge: null }],
        activeDumpId: id,
      }));
      persist();
    },
    ...
    deleteDump: (id) => {
      snap();
      set((s) => {
        const next = s.dumps.filter((d) => d.id !== id);
        return {
          dumps: next.length ? next : [{ id: 'd1', num: 1, title: 'Untitled Dump', photos: [], vibeBadge: null }],
          activeDumpId: s.activeDumpId === id ? (next[0]?.id ?? 'd1') : s.activeDumpId,
        };
      });
      persist();
    },
    ...
    updateDumpTitle: (dumpId, title) => {
      set((s) => ({
        dumps: s.dumps.map((d) => d.id === dumpId ? { ...d, title } : d),
      }));
      persist();
    },
    ...
    checkDumpVibe: (dumpId) => {
      const s = get();
      const dump = s.dumps.find(d => d.id === dumpId);
      if (!dump || dump.photos.length === 0) return;
      const photos = dump.photos.map(id => s.photos.find(p => p.id === id)).filter((p): p is Photo => !!p);
      const isConsistent = checkColorTemp(photos);
      const vibeBadge = isConsistent ? null : 'mismatch';
      set((s) => ({
        dumps: s.dumps.map(d => d.id === dumpId ? { ...d, vibeBadge } : d),
      }));
    },

    toggleDumpLike: (dumpId) => {
      set((s) => ({
        dumps: s.dumps.map((d) => d.id === dumpId ? { ...d, liked: !d.liked } : d),
      }));
      persist();
    },
    ...
    approveDumpTitle: (dumpId) => {
      set((s) => ({
        dumps: s.dumps.map((d) => d.id === dumpId ? { ...d, titleApproved: true } : d),
      }));
      persist();
    },
    ...
    rejectDumpTitle: (dumpId) => {
      ...
      if (!dump) return;
      const photos = dump.photos.map(id => get().photos.find(p => p.id === id)).filter((p): p is Photo => !!p);
      const prompt = `Generate a new creative title for this photo dump with categories: ${photos.map(p => p.category).join(', ')}. Return only the title string.`;
      fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ messages: [{ role: 'user', content: prompt }] }),
      }).then(res => res.json()).then(data => {
        const newTitle = data.choices[0].message.content.replace(/"/g, '').trim();
        if (newTitle) {
          set((s) => ({
            dumps: s.dumps.map((d) =>
...
export async function loadPhotosFromServer() {
  try {
    // 1. Re-hydrate local photos from IndexedDB
    const existing = useStore.getState().photos;
    const hydrated = await Promise.all(existing.map(async p => {
      if (p.url.startsWith('blob:')) {
        const blob = await getPhotoBlob(p.id);
        if (blob) return { ...p, url: URL.createObjectURL(blob) };
      }
      return p;
    }));
    useStore.setState({ photos: hydrated });

    // 2. Skip server load if we already have photos
